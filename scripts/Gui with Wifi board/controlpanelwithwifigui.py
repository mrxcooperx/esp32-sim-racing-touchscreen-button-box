...
WIFI_CONFIG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wifi_flag_display.json")
DEFAULT_WIFI_CONFIG = {"enabled": False, "host": "", "port": 4210}
...
PAGES_LAYOUT = [
    ...
    [    # Page 1: Reset / Tortoise / Wheel Repair / Battery
    ...
    [    # Page 2: 2x fuel can + 4x tire (tags match the sketch's current LF/RF swap)
...
PAGE_NAMES = ["Flag (blank)", "Reset/Tortoise/Repair/Battery", "Fuel & Tires", "D-Pad", "Adjusters"]

# Page button stack, mirrored on the preview too (matches the real
# left-edge buttons - same math as the firmware's pageBtnHeight/pageBtnY)
SLIDER_X, SLIDER_Y, SLIDER_W, SLIDER_H = 5, 10, 27, 220
PAGE_BTN_GAP = 4
...
def page_btn_y(page):
    return SLIDER_Y + page * (page_btn_height() + PAGE_BTN_GAP)
...
class Backend:
    ...
    def __init__(self, mapping_lock, mapping_ref, event_queue):
        ...
        # Optional wireless flag display - sends the same FLAG:/party
        # messages over UDP alongside whatever goes out over serial.
        wifi_cfg = load_wifi_config()
        self.wifi_enabled = wifi_cfg.get("enabled", False)
        self.wifi_host = wifi_cfg.get("host", "")
        self.wifi_port = wifi_cfg.get("port", 4210)
        self.udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)

    def set_wifi_target(self, enabled, host, port=4210):
        self.wifi_enabled = enabled
        self.wifi_host = host.strip()
        self.wifi_port = port
        save_wifi_config({"enabled": self.wifi_enabled, "host": self.wifi_host, "port": self.wifi_port})
    ...
    def _send_to_all(self, message):
        ...
        if self.wifi_enabled and self.wifi_host:
            try:
                self.udp_socket.sendto(message.encode("utf-8"), (self.wifi_host, self.wifi_port))
            except OSError as e:
                self.log(f"WiFi board send failed ({self.wifi_host}:{self.wifi_port}): {e}")
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # --- Optional wireless flag display ---
        ttk.Label(top_frame, text="WiFi Board:").grid(row=5, column=0, sticky="w", pady=2)
        wifi_frame = ttk.Frame(top_frame)
        wifi_frame.grid(row=5, column=1, columnspan=3, sticky="w", padx=5, pady=2)

        self.wifi_enabled_var = tk.BooleanVar(value=self.backend.wifi_enabled)
        wifi_check = tk.Checkbutton(
            wifi_frame, text="Enabled", variable=self.wifi_enabled_var,
            bg=self.cget("bg"), fg="#e6e6e6", selectcolor="#2b2b2b",
            activebackground=self.cget("bg"), activeforeground="#e6e6e6",
        )
        wifi_check.pack(side="left", padx=(0, 8))

        self.wifi_host_entry = ttk.Entry(wifi_frame, width=22)
        self.wifi_host_entry.insert(0, self.backend.wifi_host)
        self.wifi_host_entry.pack(side="left", padx=(0, 5))

        ttk.Button(wifi_frame, text="Save", command=self._save_wifi_target).pack(side="left", padx=(0, 8))
        ttk.Label(
            wifi_frame, text="IP or hostname, e.g. flagdisplay.local", font=("", 8),
        ).pack(side="left")
        ...
        log_frame = tk.Frame(status_tab, width=SCREEN_W, height=400, bg="#111111")
        log_frame.pack(padx=10, pady=(0, 10), anchor="w")
        log_frame.pack_propagate(False)
        self.log_text = tk.Text(log_frame, state="disabled", bg="#111111", fg="#dddddd", wrap="word")
    ...
    def _draw_preview_for_port(self, port):
        ...
        # Page button stack, mirroring the real left-edge buttons - one
        # per page, current page filled in, the rest just outlined.
        btn_h = page_btn_height()
        for p in range(NUM_PAGES):
            by = page_btn_y(p)
            x0, y0 = (SLIDER_X + 2) * s, by * s
            x1, y1 = (SLIDER_X + SLIDER_W - 2) * s, (by + btn_h) * s
            if p == page:
                c.create_rectangle(x0, y0, x1, y1, fill="#3498db", outline="")
            else:
                c.create_rectangle(x0, y0, x1, y1, fill="", outline="#ffffff")
        ...
        for btn in PAGES_LAYOUT[page]:
            ...
            if icon_name:
                ...
                icon_cy = (y + h / 2) - (10 * s if tag else 0)  # nudge up a bit if a tag goes below
    ...
    def _save_wifi_target(self):
        enabled = self.wifi_enabled_var.get()
        host = self.wifi_host_entry.get().strip()
        self.backend.set_wifi_target(enabled, host, self.backend.wifi_port)
        if enabled and host:
            self._append_log(f"WiFi flag board enabled -> {host}:{self.backend.wifi_port}")
        else:
            self._append_log("WiFi flag board disabled")
